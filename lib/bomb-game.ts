import { COMMAND_PROTECTION_SECONDS, COMMAND_WEIGHTS, EVENT_INTERVALS, EXPLOSION_RANGES, FAKE_SOUND_INTERVAL } from "./game-config";
...
export class BombScheduler {
  ...
  start(now = Date.now()) { this.explosionTimer = setTimeout(() => { if (!this.stopped) { this.stop(); this.events.explode(); } }, Math.max(0, this.explosionAt - now)); this.scheduleCommand(); this.scheduleFake(); }
  ...
  private scheduleCommand() { if (this.stopped || !this.settings.enabledCommands.length) return; this.commandTimer = setTimeout(() => { if (this.stopped || Date.now() >= this.explosionAt) return; const command = chooseCommand(this.settings.enabledCommands, this.previous, this.random); if (command) { this.previous = command; this.events.command(command); } this.scheduleCommand(); }, nextCommandDelayMs(this.settings, this.previous, this.random)); }
  private scheduleFake() { if (this.stopped || !this.settings.fakeSounds) return; this.fakeTimer = setTimeout(() => { if (this.stopped || Date.now() >= this.explosionAt) return; this.events.fake(); this.scheduleFake(); }, nextFakeDelayMs(this.random)); }
}
